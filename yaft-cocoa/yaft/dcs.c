...
static inline int sixel_repeat(struct terminal_t *term, struct sixel_canvas_t *sc, char *buf)
{
	...
	*(tmp + length - 1) = '\0';
	...
	logging(LOG_DEBUG, "sixel_repeat()\nbuf:%s length:%u\ncount:%d repeat:0x%.2X\n",
...
static inline int sixel_color(struct sixel_canvas_t *sc, char *buf)
{
	...
	*(tmp + length - 1) = '\0';
	...
	logging(LOG_DEBUG, "type:%d v1:%u v2:%u v3:%u color:0x%.8X\n",
		type, v1, v2, v3, color);
...
void sixel_parse_data(struct terminal_t *term, struct sixel_canvas_t *sc, char *start_buf)
{
	...
	*/
	...
	logging(LOG_DEBUG, "sixel_parse_data()\nwidth:%d height:%d\n", sc->width, sc->height);
}
...
void sixel_parse_header(struct terminal_t *term, char *start_buf)
{
	...
	*cp = '\0';
	...
	logging(LOG_DEBUG, "sixel_parse_header()\nbuf:%s\n", start_buf);
	...
	for (i = 0; i < BITS_PER_SIXEL; i++) {
		if((bitmap >> i) & 0x01) {
			...
			if (height_shift < CELL_HEIGHT) {
				logging(LOG_DEBUG, "height_shift:%d width_shift:%d\n", height_shift, width_shift);
	...
	*/
	...
	while (cp < end_buf) {
		if ('?' <= *cp && *cp <= '~') { /* sixel bitmap */
			logging(LOG_DEBUG, "char_num(ten):0x%.2X\n", char_num);
...
void decdld_parse_header(struct terminal_t *term, char *start_buf)
{
	...
	*cp = '\0';
	...
	logging(LOG_DEBUG, "decdld_parse_header()\nbuf:%s\n", start_buf);
	...
	logging(LOG_DEBUG, "charset(ku):0x%.2X start_char:%d erase_mode:%d\n",
